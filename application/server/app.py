...
class RunState(TypedDict):
    ...
    images: list[
        dict[str, str]
    ]  # e.g. {"url": "/static/<id>/offense.png", "caption": "..."}
...
def _get_or_build_ui_summary(
    token: str, mode: PlotMode, player_name: Optional[str] = None
) -> UISummary:
    """
    ...
    # Debug logging
    print(
        f"[DEBUG _get_or_build_ui_summary] Computed summary for mode={mode}, player={cache_key}"
    )
    print(
        "[DEBUG _get_or_build_ui_summary] "
        f"avg_errors_per_set = {ui.get('avg_errors_per_set', 'NOT FOUND')}"
    )
...
def upload():
    ...
    try:
        _purged = purge_dirs(dirs=[UPLOADS_DIR, STATIC_DIR], keep=keep_names)
    except Exception:
        _purged = 0  # fail-open; don't block upload

    print(f"[DEBUG] Purged {_purged} files.")
    ...
    # Single read path (via services.io_loader) OR use read_tablelike directly.
    try:
    ...
    try:
        ...
        return jsonify(
            {"error": f"failed to compute metrics: {e}", "have": list(df.columns)}
        ), 400
    ...
    _images = generate_plots(
        df, out_dir=str(app.static_folder), run_id=run_id, summary=_profile
    )
    ...
    return jsonify(
        {"run_id": run_id, "summary": ui_summary, "meta": meta, "images": _images}
    )
    ...
    if query is not None:
        try:
            ...
            resolved = {
                "original": query,
                "used_name": used,
                "exact": used.lower() == query.strip().lower() if used else False,
            }
        except Exception as e:
...
def summary_api():
    ...
    player = (
        request.args.get("player", "") or ""
    ).strip()  # "all" | "team" | specific name

    # "offense" | "service" | "receive" |
    mode: PlotMode = coerce_plot_mode(request.args.get("mode", ""))
...
def plot_endpoint():
    ...
    print(
        f"DEBUG calling render_plot with: kind='{kind}', mode='{plot_mode}', player='{used}'"
    )
...
def commentary():
    ...
    if not all_plots:
        response, status = build_error_response(
            "No plots could be generated for analysis", "NO_PLOTS"
        )
