...
def resolve_player_name(
    df: pd.DataFrame, raw_name: Optional[str]
...
def get_player_data(df: pd.DataFrame, player_name: str) -> Tuple[pd.DataFrame, str]:
    ...
    if result is None:
        raise ValueError(f"Error: '{player_name}' not found. No close match found.")
    ...
    raise ValueError(
        f"Error: '{player_name}' not found. "
        f"Closest candidate '{closest_match}' "
        f"scored {score} (< {THRESHOLD})."
    )
...
def add_accuracy_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # --- derived totals ---
    df["atk_total"] = (
        _num("atk_error") + _num("atk_bad") + _num("atk_good") + _num("atk_kill")
    )
    df["srv_total"] = _num("srv_error") + _num("srv_good") + _num("srv_ace")
    df["rcv_total"] = (
        _num("rcv_error") + _num("rcv_bad") + _num("rcv_good") + _num("rcv_perfect")
    )
    df["blk_total"] = (
        _num("blk_solo") + _num("blk_assist_2") / 2 + _num("blk_assist_3") / 3
    )
    ...
    df["rcv_accuracy"] = _safe_div(
        3 * rcv_perf + 2 * rcv_good + rcv_bad, 3 * rcv_total
    ).round(3)
    df["atk_accuracy"] = _safe_div(atk_kill - atk_error, atk_total).round(3)
    df["total_errors"] = (
        srv_error + rcv_err + atk_error + blk_error + def_error
    ).astype(float)
...
def compute_summary(
    df: pd.DataFrame, mode: PlotMode, player_name: Optional[str] = None
) -> UISummary:
    ...
    work = (
        df_work
        if no_filter or "player_name" not in df_work.columns
        else df_work[df_work["player_name"].astype(str).str.strip().str.lower() == name]
    )

    print(
        f"[DEBUG compute_summary] games_count exists: "
        f"{True if 'games_count' in work.columns else False}"
    )
    ...
    srv_accuracy: float = (
        round((srv_total - srv_error) / srv_total, 3) if srv_total > 0 else 0.0
    )
    ...
    rcv_accuracy: float = (
        round(rcv_num / (3.0 * rcv_total), 3) if rcv_total > 0 else 0.0
    )
    ...
    atk_accuracy: float = (
        round((atk_kill - atk_error) / atk_total, 3) if atk_total > 0 else 0.0
    )
    ...
    total_games = (
        float(work["games_count"].fillna(0).sum())
        if "games_count" in work.columns
        else 0.0
    )
    avg_errors_per_set: float = (
        round(total_errors / total_games, 3) if total_games > 0 else 0.0
    )

    # Debug logging
    print(f"[DEBUG compute_summary] Mode: {mode}, Player: {player_name}")
    print(
        f"[DEBUG compute_summary] Total errors: {total_errors}, Total games: {total_games}"
    )
...
def build_cumulative(df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    ...
    df_grouped = (
        ...
        .groupby(
            ["date", "format", "season", "week_num", "player_name", "year", "position"]
        )
        .sum()
        .reset_index()
    )
    ...
    df_overall = (
        df_grouped.drop(
            ["date", "week_num", "position", "year", "format", "season"], axis=1
        )
        .groupby(["player_name"])
        .sum()
        .reset_index()
    ...
    # Debug
    print(
        f"[DEBUG build_cumulative] games_count sum: {df_grouped['games_count'].sum()}"
    )
...
def build_temporal(df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    ...
    team_overall = (
        ...
        .groupby(["date", "year", "player_name", "week_num"])
        .sum()
        .reset_index()
    )
    team_overall = (
        team_overall.drop(["player_name"], axis=1)
        .groupby(["date", "year", "games_count", "week_num"])
        .sum()
        .reset_index()
    )
...
def compute_temporal_statistics(
    df: pd.DataFrame, player_name: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    if len(df) > 1:
        ...
        if "win" in df.columns:
            ...
            stats["overall_win_rate"] = (
                float(total_wins / total_games) if total_games > 0 else 0
            )
    ...
    if not df.empty:
        ...
        stats["last_game"] = (
            df["date"].max().isoformat() if pd.notna(df["date"].max()) else None
        )
...
def compute_cumulative_statistics(
    ...
    stats["total_games"] = (
        int(df["games_count"].sum()) if "games_count" in df.columns else 0
    )
    ...
    if "srv_total" in df.columns:
        ...
        stats["total_srv_errors"] = (
            int(df["srv_error"].sum()) if "srv_error" in df.columns else 0
        )
        stats["ace_percentage"] = (
            float(stats["total_aces"] / stats["total_serves"])
    ...
    if "rcv_total" in df.columns:
        ...
        stats["total_perfect_passes"] = (
            int(df["rcv_perfect"].sum()) if "rcv_perfect" in df.columns else 0
        )
        stats["total_rcv_errors"] = (
            int(df["rcv_error"].sum()) if "rcv_error" in df.columns else 0
        )
        stats["perfect_pass_percentage"] = (
            float(stats["total_perfect_passes"] / stats["total_receives"])
            if stats["total_receives"] > 0
            else 0
        )
    ...
    if "atk_total" in df.columns:
        ...
        stats["total_kills"] = (
            int(df["atk_kill"].sum()) if "atk_kill" in df.columns else 0
        )
        stats["total_atk_errors"] = (
            int(df["atk_error"].sum()) if "atk_error" in df.columns else 0
        )
        stats["kill_percentage"] = (
            float(stats["total_kills"] / stats["total_attacks"])
    ...
    if stats["total_games"] > 0:
        for key in [
            ...
            if key in stats:
                stats[f"{key.replace('total_', 'avg_')}_per_game"] = round(
                    stats[key] / stats["total_games"], 2
                )
