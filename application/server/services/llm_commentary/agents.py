...
def call_hf_api(
    model_id: str, prompt: str, hf_token: str, max_tokens: int = 500
) -> Optional[str]:
...
def _load_vision_model(
    model_id: str = "HuggingFaceTB/SmolVLM-Instruct", hf_token: Optional[str] = None
) -> Tuple[Any, Any]:
    ...
    _VISION_MODEL = AutoModelForImageTextToText.from_pretrained(  # type: ignore
        model_id, torch_dtype=dtype, **common_kwargs
    )
...
def _load_model_and_tokenizer(
    model_id: str, hf_token: Optional[str]
) -> Tuple[Any, Any]:
    ...
    try:
        ...
        for fallback_id in fallback_models:
            try:
                ...
                _TOKENIZER = AutoTokenizer.from_pretrained(  # type: ignore
                    fallback_id,
                    **common_kwargs
                )
                _MODEL = AutoModelForCausalLM.from_pretrained(  # type: ignore
                    fallback_id, torch_dtype=dtype, **common_kwargs
                )
        ...
        if _MODEL is None or _TOKENIZER is None:
            raise RuntimeError(
                "Could not load any model. Please check your token and model access."
            )
...
def build_agent_context(
    ...
    parts.append(
        f"=== VOLLEYBALL TEAM PERFORMANCE DATA ({analysis_mode.upper()} ANALYSIS) ===\n"
    )
    ...
    parts.append("  • This is the ACTUAL measured service accuracy from the data")
    ...
    parts.append("  • This is the ACTUAL measured receive accuracy from the data")
    ...
    parts.append("  • This is the ACTUAL hitting percentage from the data")
    ...
    parts.append("  • This is the ACTUAL measured error rate from the data")
    ...
    if analysis_mode == "temporal" and mode_stats:
        ...
        if "srv_trend" in mode_stats:
            ...
            parts.append("\nService Trends:")
            parts.append(
                f"  • Trend: {trend_dir} ({mode_stats['srv_trend']:.4f} per game)"
            )
            parts.append(
                f"  • Consistency (std dev): {mode_stats.get('srv_consistency', 0):.3f}"
            )
            parts.append(
                f"  • Recent 3-game avg: {mode_stats.get('srv_recent_avg', 0) * 100:.1f}%"
            )
            parts.append(
                f"  • Best game: {mode_stats.get('srv_best_game', 0) * 100:.1f}%"
            )
            parts.append(
                f"  • Worst game: {mode_stats.get('srv_worst_game', 0) * 100:.1f}%"
            )
        ...
        if "rcv_trend" in mode_stats:
            ...
            parts.append("\nReceive Trends:")
            parts.append(
                f"  • Trend: {trend_dir} ({mode_stats['rcv_trend']:.4f} per game)"
            )
            parts.append(
                f"  • Consistency (std dev): {mode_stats.get('rcv_consistency', 0):.3f}"
            )
            parts.append(
                f"  • Recent 3-game avg: {mode_stats.get('rcv_recent_avg', 0) * 100:.1f}%"
            )
            parts.append(
                f"  • Best game: {mode_stats.get('rcv_best_game', 0) * 100:.1f}%"
            )
            parts.append(
                f"  • Worst game: {mode_stats.get('rcv_worst_game', 0) * 100:.1f}%"
            )
        ...
        if "atk_trend" in mode_stats:
            ...
            parts.append("\nAttack Trends:")
            parts.append(
                f"  • Trend: {trend_dir} ({mode_stats['atk_trend']:.4f} per game)"
            )
            parts.append(
                f"  • Consistency (std dev): {mode_stats.get('atk_consistency', 0):.3f}"
            )
            parts.append(
                f"  • Recent 3-game avg: {mode_stats.get('atk_recent_avg', 0):.3f}"
            )
        ...
        if "error_trend" in mode_stats:
            trend_dir = (
                "improving" if mode_stats["error_trend"] < 0 else "worsening"
            )  # Negative is good for errors
            parts.append("\nError Trends:")
            parts.append(
                f"  • Trend: {trend_dir} ({abs(mode_stats['error_trend']):.3f} per game)"
            )
            parts.append(
                f"  • Recent 3-game avg: {mode_stats.get('error_recent_avg', 0):.2f} errors/set"
            )
            parts.append(
                f"  • Best game: {mode_stats.get('error_best_game', 0):.2f} errors/set"
            )
            parts.append(
                f"  • Worst game: {mode_stats.get('error_worst_game', 0):.2f} errors/set"
            )
        ...
        if "momentum" in mode_stats:
            ...
            parts.append(
                f"  • Recent win rate: {mode_stats.get('recent_win_rate', 0) * 100:.1f}%"
            )
            parts.append(
                f"  • Overall win rate: {mode_stats.get('overall_win_rate', 0) * 100:.1f}%"
            )
            parts.append(
                f"  • Momentum: {'+' if mode_stats['momentum'] > 0 else ''}"
                f"{mode_stats['momentum'] * 100:.1f}%"
            )
        ...
        if "total_serves" in mode_stats:
            ...
            parts.append(
                f"  • Total aces: {mode_stats.get('total_aces', 0)} "
                f"({mode_stats.get('ace_percentage', 0) * 100:.1f}%)"
            )
            parts.append(f"  • Total errors: {mode_stats.get('total_srv_errors', 0)}")
            parts.append(
                f"  • Avg serves/game: {mode_stats.get('avg_serves_per_game', 0):.1f}"
            )
            parts.append(
                f"  • Avg aces/game: {mode_stats.get('avg_aces_per_game', 0):.1f}"
            )
        ...
        if "total_receives" in mode_stats:
            ...
            parts.append(
                f"  • Perfect passes: {mode_stats.get('total_perfect_passes', 0)} "
                f"({mode_stats.get('perfect_pass_percentage', 0) * 100:.1f}%)"
            )
        ...
        if "total_attacks" in mode_stats:
            ...
            parts.append(
                f"  • Total kills: {mode_stats.get('total_kills', 0)} "
                f"({mode_stats.get('kill_percentage', 0) * 100:.1f}%)"
            )
            parts.append(f"  • Total errors: {mode_stats.get('total_atk_errors', 0)}")
            parts.append(
                f"  • Avg attacks/game: {mode_stats.get('avg_attacks_per_game', 0):.1f}"
            )
            parts.append(
                f"  • Avg kills/game: {mode_stats.get('avg_kills_per_game', 0):.1f}"
            )
        ...
        if "total_assists" in mode_stats:
            ...
            parts.append(
                f"  • Avg assists/game: {mode_stats.get('avg_assists_per_game', 0):.1f}"
            )

    # Add interpretation guidelines
    parts.append("\n=== IMPORTANT INSTRUCTIONS ===")
    parts.append(
        "1. USE ONLY THE NUMBERS PROVIDED ABOVE - DO NOT MAKE UP ANY STATISTICS"
    )
    parts.append(
        "2. All percentages and values are already calculated - use them exactly as shown"
    )
    parts.append(
        "3. When discussing player performance, only mention players from the list above"
    )
    parts.append(
        "4. Base your analysis on the actual data values provided, not assumptions"
    )
    parts.append(
        f"5. This is {analysis_mode.upper()} analysis - focus on "
        f"{'trends over time' if analysis_mode == 'temporal' else 'overall totals and averages'}"
    )
    ...
    if images:
        ...
        for i, img in enumerate(images):
            if isinstance(img, dict):
                parts.append(
                    f"  - {img.get('name', f'Image {i + 1}')}: {img.get('url', 'available')}"
                )
    ...
    if plot_images:
        ...
        for i, img in enumerate(plot_images):
            ...
            parts.append(
                f"  - Plot {i + 1}: {meta_info.get('kind', 'Unknown')} for "
                f"{meta_info.get('player', 'team')} ({meta_info.get('mode', 'unknown')} view)"
            )
...
def visually_analyze_plot(
    ...
    try:
        # Load the VLM
        model, processor = _load_vision_model(model_id=model_id, hf_token=hf_token)
        ...
        messages = [
            {
                ...
                "content": [
                    {"type": "image", "image": img},
                    {
                        ...
                        - Service Accuracy: {summary.get("srv_accuracy", 0):.3f}
                        ({summary.get("srv_accuracy", 0) * 100:.1f}%)
                        - Receive Accuracy: {summary.get("rcv_accuracy", 0):.3f}
                        ({summary.get("rcv_accuracy", 0) * 100:.1f}%)
                        ...
                        IMPORTANT: Use only the statistics provided above. "
        ...
        inputs = {
            k: v.to(device) if torch.is_tensor(v) else v for k, v in inputs.items()  # type: ignore
        }  # type: ignore
        ...
        generated_response = processor.batch_decode(
            ...
            if inputs["input_ids"].size(1) < generated_ids.size(1)
            else generated_ids,
...
def generate_commentary_with_plots(
    ...
    context = build_agent_context(
        summary=summary, meta=meta, images=images, plot_images=plot_images
    )
    ...
    system = (
        ...
        "When you see 'Service Accuracy: 0.844 (84.4%)', you must use exactly "
        "84.4%, not any other value. "
        "Similarly for all other metrics. Your analysis should be based solely "
    ...
    user_parts = ["Volleyball Team Analysis Report\n", f"{context}\n"]
    ...
    pipe, tok = _load_model_and_tokenizer(model_id=text_model_id, hf_token=hf_token)
...
def generate_basic_statistical_commentary(
    summary: UISummary, mode: str, player: str
) -> str:
    """
    ...
    if mode.lower() == "temporal":
        header = (
            f"Temporal Statistical Analysis for {player_name} - Performance Trends\n"
        )
    else:
        header = (
            f"Cumulative Statistical Analysis for {player_name} - Overall Performance\n"
        )
    ...
    if srv_acc > 0.9:
        insights.append(
            f"• Excellent service accuracy at {srv_acc:.1%} - "
            "well above competitive standards"
        )
    elif srv_acc > 0.85:
        insights.append(
            f"• Good service accuracy at {srv_acc:.1%} - meeting competitive standards"
        )
    else:
        insights.append(
            f"• Service accuracy at {srv_acc:.1%} needs improvement to "
            "reach 85%+ competitive standard"
        )

    # Receive analysis
    if rcv_acc > 0.75:
        insights.append(
            f"• Strong serve-receive performance at {rcv_acc:.1%} - "
            "creating good offensive opportunities"
        )
    elif rcv_acc > 0.65:
        insights.append(
            f"• Adequate receive accuracy at {rcv_acc:.1%} - room for improvement"
        )
    else:
        insights.append(
            f"• Receive accuracy at {rcv_acc:.1%} is limiting offensive options "
    ...
    if atk_acc > 0.3:
        insights.append(
            f"• Outstanding hitting percentage at {atk_acc:.3f} - elite offensive performance"
        )
    elif atk_acc > 0.2:
        insights.append(
            f"• Solid hitting percentage at {atk_acc:.3f} - competitive offensive output"
        )
    else:
        insights.append(
            f"• Hitting percentage at {atk_acc:.3f} indicates offensive struggles"
        )

    # Error analysis
    if errors < 4:
        insights.append(
            f"• Excellent error control at {errors:.1f} per set - maintaining discipline"
        )
    elif errors < 6:
        insights.append(
            f"• Acceptable error rate at {errors:.1f} per set - some room for improvement"
        )
    else:
        insights.append(
            f"• High error rate at {errors:.1f} per set is giving away too many points"
        )
    ...
    if mode.lower() == "temporal" and mode_stats:
        ...
        if "error_trend" in mode_stats:
            ...
            if err_trend < -0.01:
                insights.append(
                    f"• Errors improving (decreasing): {abs(err_trend):.3f} fewer per game"
                )
            elif err_trend > 0.01:
                insights.append(
                    f"• Errors worsening (increasing): +{err_trend:.3f} more per game"
                )
            else:
        ...
        if "momentum" in mode_stats and "recent_win_rate" in mode_stats:
            ...
            if momentum > 0.1:
                insights.append(
                    f"• Strong positive momentum: Recent wins ({recent_wr:.0f}%) "
                    f"exceed season average ({overall_wr:.0f}%)"
                )
            elif momentum < -0.1:
                insights.append(
                    f"• Negative momentum: Recent wins ({recent_wr:.0f}%) "
                    f"below season average ({overall_wr:.0f}%)"
                )
            else:
                insights.append(
                    f"• Steady performance: Recent and overall win rates aligned "
                    f"(~{overall_wr:.0f}%)"
                )
        ...
        if "total_games" in mode_stats:
            ...
            insights.append(
                f"• Record: {total_wins}-{total_losses} ({win_pct:.1f}% win rate) "
        ...
        if "total_serves" in mode_stats:
            ...
            insights.append(
                f"• Service: {total_serves} total serves with {total_aces} aces ({ace_pct:.1f}%)"
            )
        ...
        if "total_attacks" in mode_stats:
            ...
            insights.append(
                f"• Offense: {total_attacks} total attacks with {total_kills} "
                f"kills ({kill_pct:.1f}%)"
            )
        ...
        if "total_receives" in mode_stats:
            ...
            insights.append(
                f"• Receive: {total_rcv} total passes with {perfect_passes} "
        ...
        if "total_assists" in mode_stats:
            ...
            insights.append(
                f"• Setting: {mode_stats['total_assists']} total assists "
                f"({avg_assists:.1f} per game)"
            )
    ...
    if mode.lower() == "temporal" and mode_stats:
        ...
        if mode_stats.get("srv_trend", 0) < -0.001:
            recommendations.append(
                "• Service trending down - review technique and mental approach"
            )
        elif srv_acc < 0.85:
            recommendations.append(
                "• Focus on consistent toss placement in serve practice"
            )

        if mode_stats.get("rcv_trend", 0) < -0.001:
            recommendations.append(
                "• Receive declining - increase passing reps and footwork drills"
            )
        elif rcv_acc < 0.65:
            recommendations.append(
                "• Prioritize serve-receive with emphasis on platform control"
            )

        if mode_stats.get("atk_trend", 0) < -0.001:
            recommendations.append(
                "• Attack efficiency declining - review approach angles and timing"
            )
        elif atk_acc < 0.2:
        ...
        if mode_stats.get("error_trend", 0) > 0.01:
            recommendations.append(
                "• Errors increasing - implement controlled scrimmages for consistency"
            )
        elif errors > 6:
            recommendations.append(
                "• High error baseline - focus on ball control fundamentals"
            )

        if mode_stats.get("momentum", 0) < -0.1:
            recommendations.append(
                "• Address negative momentum with team building and mental training"
            )
        ...
        if srv_acc < 0.85:
            ...
            if mode_stats.get("ace_percentage", 0) < 0.05:
                recommendations.append(
                    "• Develop more aggressive serving options for ace opportunities"
                )

        if rcv_acc < 0.65:
            recommendations.append(
                "• Increase serve-receive reps with emphasis on platform angle"
            )
            if mode_stats.get("perfect_pass_percentage", 0) < 0.3:
                recommendations.append(
                    "• Work on passing accuracy to enable better offensive sets"
                )

        if atk_acc < 0.2:
            recommendations.append(
                "• Work on shot selection and identifying open court areas"
            )
            if mode_stats.get("kill_percentage", 0) < 0.25:
                recommendations.append(
                    "• Develop power and placement for higher kill conversion"
                )

        if errors > 6:
            recommendations.append(
                "• Implement 'zero error' drills focusing on ball control"
            )
        ...
        if mode_stats.get("total_games", 0) > 0:
            ...
            if win_rate < 0.4:
                recommendations.append(
                    "• Focus on mental toughness and closing tight sets"
                )

    # Add footer
    footer = (f"\n\nNote: Statistical summary based on {summary.get('rows', 0)} "
              f"data points from {summary.get('date_min', 'N/A')} to "
              f"{summary.get('date_max', 'N/A')}.")
...
def generate_commentary_with_api(
        ...
        if isinstance(plot_data, list):
            for plot in plot_data:
                ...
                if plot_type == "offense":
                    desc = (
                        f"Offensive performance plot shows {summary.get('kills', 0)} kills "
                        f"with {summary.get('atk_accuracy', 0) * 100:.1f}% accuracy"
                    )
                elif plot_type == "service":
                    desc = (
                        f"Service plot indicates {summary.get('aces', 0)} aces "
                        f"and {summary.get('srv_accuracy', 0) * 100:.1f}% accuracy"
                    )
                elif plot_type == "errors":
                    desc = (
                        f"Error analysis shows "
                        f"{summary.get('avg_errors_per_set', 0):.1f} average errors per set"
                    )
                else:
    ...
    if plot_descriptions:
        context += "\n\nVisualization Analysis:\n" + "\n".join(
            [f"- {desc}" for desc in plot_descriptions]
        )
    ...
    # Build user prompt
    user_parts = [
        "Volleyball Team Analysis Report\n",
        f"{context}\n",
        "\n\nKEY METRICS:",
        f"\n• Service Accuracy: {summary.get('srv_accuracy', 0) * 100:.1f}%",
        f"\n• Receive Accuracy: {summary.get('rcv_accuracy', 0) * 100:.1f}%",
    ...
    text = call_hf_api(
        model_id=model_id, prompt=prompt, hf_token=hf_token, max_tokens=max_new_tokens
    )
...
def generate_commentary(
    ...
    if not hf_token:
        return (
            "Commentary unavailable - no API token provided",
