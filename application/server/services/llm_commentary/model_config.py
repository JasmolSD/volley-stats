...
def validate_commentary_request(
    token: str, run_cache: Dict
...
def generate_plots_for_mode(
    ...
    # Resolve player name
    filtered_df, used_player = (
        resolve_player_name_func(df, player) if player else (df, "")
    )
    ...
    try:
        ...
        for fallback_text in FALLBACK_TEXT_MODELS:
            for fallback_vision in FALLBACK_VISION_MODELS:
                if (
                    fallback_text == DEFAULT_TEXT_MODEL
                    and fallback_vision == DEFAULT_VISION_MODEL
                ):
                ...
                try:
                    commentary_text, fallback_text, fallback_vision = (
                        generate_commentary(
                            summary=ui_summary,
                            meta=meta,
                            images=images,
                    ...
                    if commentary_text and not commentary_text.startswith(
...
def build_commentary_response(
    ...
    if is_fallback:
        response["note"] = (
            "Generated using statistical analysis (AI models unavailable)"
        )
